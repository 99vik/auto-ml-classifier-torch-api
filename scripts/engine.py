...
def engine(file, label_index, iterations, learning_rate, activation_function, progress_queue, hidden_layers, normalization, train_ratio, dropout):

    try:
        labels, data, data_by_labels = csv_parser(label_index=label_index, file=file)
        X_tr, X_te, y_tr, y_te = to_split_tensor_data(labels, data, label_index, train_ratio)
        X_tr = X_tr.to(device)
        X_te = X_te.to(device)
        y_tr = y_tr.to(device)
        y_te = y_te.to(device)
